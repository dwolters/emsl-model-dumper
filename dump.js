...
async function getModelNames() {
    ...
    // Retrieve all models in Datebase
    let results = await session.run('MATCH (n:NeoCore__Model) WHERE NOT n:NeoCore__MetaModel RETURN n.ename');
    let modelNames = results.records.map(r => r.get(0));
    session.close();
    return modelNames;
}
...
async function getNodes(modelName) {
    ...
    let results = await session.run(`
        ...
        WITH id(n) as id, labels(n)[size(labels(n))-1] as type, labels(n) as types, properties(n) as props
        ORDER BY type, id
